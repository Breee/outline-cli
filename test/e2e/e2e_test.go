...
func extractFormAction(htmlContent string, baseURL *url.URL) string {
	// Simple extraction — find action="..." in form tag.
	idx := strings.Index(htmlContent, "<form")
	if idx == -1 {
		return ""
	}
	formTag := htmlContent[idx:]
...
func TestExtractFormActionUnescapesHTMLEntities(t *testing.T) {
	baseURL, err := url.Parse("http://dex:5556/dex/auth/local/login")
	if err != nil {
		t.Fatalf("parse base url: %v", err)
	}

	formHTML := `<form action="/dex/auth/local/login?back=&amp;state=is4ydxmpqi2dm7uysxlmjjk7c" method="post">`
	got := extractFormAction(formHTML, baseURL)
	want := "http://dex:5556/dex/auth/local/login?back=&state=is4ydxmpqi2dm7uysxlmjjk7c"
	if got != want {
		t.Fatalf("unexpected action url\ngot:  %s\nwant: %s", got, want)
	}
}
